...
ipcMain.handle('get-settings', () => {
    return {
        ...
        hoverToShowTodoList: store.get('settings.hoverToShowTodoList', true),
...
ipcMain.handle('save-settings', (event, newSettings) => {
    ...
    // Handle Hover to Show Todo List
    if (newSettings.hoverToShowTodoList !== undefined) {
        store.set('settings.hoverToShowTodoList', newSettings.hoverToShowTodoList);
        // Notify floating ball to update behavior
        if (floatingBallWindow) {
            floatingBallWindow.webContents.send('settings-updated', {
                hoverToShowTodoList: newSettings.hoverToShowTodoList
            });
        }
    }
    ...
    if (newSettings.language !== undefined) {
        store.set('settings.language', newSettings.language);
        // Notify all windows to update language
        const languageUpdate = { language: newSettings.language };
        if (floatingBallWindow) {
            floatingBallWindow.webContents.send('settings-updated', languageUpdate);
        }
        if (noteManagerWindow) {
            noteManagerWindow.webContents.send('settings-updated', languageUpdate);
        }
        if (quickNoteWindow) {
            quickNoteWindow.webContents.send('settings-updated', languageUpdate);
        }
    }
...
// Fetch AI Product Trends by running Python scraper
ipcMain.handle('fetch-ai-product-trends', async () => {
    return new Promise((resolve, reject) => {
        ...
        if (!fs.existsSync(scriptPath)) {
            return reject(new Error('Scraper script not found: scrape_ai_tools.py'));
        }

        // Execute Python script with image path argument
        ...
        exec(`${pythonCmd} "${scriptPath}" --images-dir "${imagesDir}"`, {
            ...
            if (!markdownContent) {
                return reject(new Error('No content generated by scraper'));
            }

            // Create note with the scraped content
            const notes = store.get('notes') || [];
            const today = new Date();
            const dateStr = today.toISOString().split('T')[0]; // YYYY-MM-DD
            ...
            const newNote = {
                id: uuidv4(),
                title: `AI Product Trends - ${dateStr}`,
...
// Translate note content (English to Chinese) using Modelscope
ipcMain.handle('translate-note-content', async (event, content) => {
    const settingsApiKey = store.get('settings.modelscopeApiKey', '');
    const apiKey = settingsApiKey || process.env.MODELSCOPE_API_KEY;

    if (!apiKey) {
        throw new Error('Modelscope API Key not configured. Please check settings.');
    }

    // Helper to call translation API with retry logic
    async function translateTextChunk(text, retries = 3) {
            ...
            try {
                const response = await fetch('https://api-inference.modelscope.cn/v1/chat/completions', {
                    ...
                    body: JSON.stringify({
                        ...
                        messages: [{
                            role: 'user',
                            content: `You are a professional translator. Translate the following text from English to Simplified Chinese. 
Rules:
1. Maintain the original tone and formatting.
2. Do NOT translate technical terms, product names, or variable names if they are commonly used in English.
3. Output ONLY the translated text, no explanations or introductory text.
4. Keep Markdown formatting characters (like *, **, #, >) intact and in the correct position.
...
${text}`
    ...
    // Split content by paragraphs to handle safely
    const lines = content.split('\n');
    const processedLines = [];

    // Patterns
    const imagePattern = /!\[.*?\]\(.*?\)/;
    const linkOnlyPattern = /^\[.*?\]\(.*?\)$/;
    ...
    async function flushBuffer() {
        ...
        const textToTranslate = buffer.join('\n');
        if (/[a-zA-Z]/.test(textToTranslate)) {
            ...
            const translated = await translateTextChunk(textToTranslate);
            processedLines.push(translated);
        } else {
            processedLines.push(textToTranslate);
        }
        buffer = [];
    }
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        // Skip separators or special blocks immediately
        if (line.trim() === '---') {
            await flushBuffer();
        ...
        // Increased buffer size to reduce number of API calls
        if (buffer.join('\n').length > 2000 || buffer.length >= 30) {
            await flushBuffer();
        }
    }
