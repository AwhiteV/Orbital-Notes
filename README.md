...
- **� AI 产品新趋势**：内置 Python 爬虫，一键抓取全网最新 AI 产品动态，自动生成 Markdown 格式的趋势报告。
...
### 使用方式
1. **直接启动（推荐）**：
   下载并运行 `Orbital Notes Setup 1.1.6.exe` 即可一键安装并开启应用。
2. **开发者模式**：
   - 克隆或下载本项目。
...
### v1.1.6 (2026-01-16)
- **多语言支持**：
    - **中英文切换**：新增语言设置选项，支持在设置界面切换中文/英文界面。
    - **实时生效**：语言切换无需重启，界面标签自动更新。
    - **全局覆盖**：支持笔记管理器、快速笔记、悬浮球 Todo List 等多界面翻译。
- **AI 产品新趋势**：
    - **自动抓取**：集成 Python 爬虫脚本，自动从热门 AI 聚合平台抓取最新 AI 产品。
    - **动态笔记**：抓取的内容自动整理为 Markdown 笔记，包含产品描述与链接。
- **悬停控制**：新增"悬停显示待办列表"开关选项，可在设置中关闭悬停展开功能。
- **安装包发布**：首次提供 NSIS 安装程序，支持自定义安装目录。
